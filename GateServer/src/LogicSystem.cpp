...
LogicSystem::LogicSystem()
{
    ...
    RegisterPost("/get_varifycode", [](std::shared_ptr<HttpConnection> connection) {
        ...
        GetVarifyRsponse rsponse = VarifyGrpcClient::GetInstance()->GetVarifyCode(email);
        std::cout << "email is " << email << std::endl;
        root["error"] = rsponse.error();
        root["email"] = src_root["email"];
        root["code"] = rsponse.code();
        std::string jsonstr = root.toStyledString();
        beast::ostream(connection->_response.body()) << jsonstr;
        return true; 
    });
    ...
    // Client sends "passwd" (xorString'ed), not "password"
    ...
    // Keep consistent with reset_pwd: verification codes are stored with CODEPREFIX.
    ...
    root["error"] = 0;
    root["email"] = src_root["email"];
    // Client debug log reads jsonObj["uid"] on register success.
    root["uid"] = uid;
